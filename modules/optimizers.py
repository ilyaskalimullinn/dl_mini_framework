...
class SGD(Optimizer):
    ...
    def step(self):
        ...
        for param, grad, m in zip(parameters, gradients, self.state['m']):
            ...
            np.add(param, self.lr * m * (-1), out=param)
...
class Adam(Optimizer):
    ...
    def step(self):
        ...
        for param, grad, m, v in zip(parameters, gradients, self.state['m'], self.state['v']):
            ...
            np.add(self.beta2 * v, (1 - self.beta2) * np.square(grad), out=v)

            m_norm = m / (1 - self.beta1 ** t)
            v_norm = v / (1 - self.beta2 ** t)

            # grad_step = -self.lr * m_norm / (np.sqrt(v_norm) + self.eps)
            grad_step = -self.lr * m / (np.sqrt(v_norm) + self.eps) / (1 - self.beta1 ** t)
            np.add(param, grad_step, out=param)
